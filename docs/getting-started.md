...
description: "How to install 239 Claude Code skills and agent plugins for 12 AI coding tools. Step-by-step setup for Claude Code, OpenAI Codex, Gemini CLI, OpenClaw, Cursor, Aider, Windsurf, and more."
---
...
| **Engineering POWERFUL** | `/plugin install engineering-advanced-skills@claude-code-skills` | 49 |
...
All 333 tools use the standard library only — zero pip installs, all verified.
...
??? question "Does this work with Cursor, Windsurf, Aider, or other tools?"
    Yes. All 239 skills can be converted to native formats for Cursor, Aider, Kilo Code, Windsurf, OpenCode, Augment, and Antigravity. Run `./scripts/convert.sh --tool all` and then install with `./scripts/install.sh --tool <name>`. See [Multi-Tool Integrations](integrations.md) for details.
