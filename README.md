...
[![Commands](https://img.shields.io/badge/Commands-31-orange?style=for-the-badge)](#commands)
...
- **Python tools** — 333 CLI scripts (all stdlib-only, zero pip installs)
- **Reference docs** — 453 templates, checklists, and domain-specific guides

**One repo, eleven platforms.** Works natively as Claude Code plugins, Codex agent skills, Gemini CLI skills, and converts to 8 more tools via `scripts/convert.sh`. All 333 Python tools run anywhere Python runs.
...
/plugin install engineering-advanced-skills@claude-code-skills  # 49 POWERFUL-tier
...
| **⚡ Engineering — POWERFUL** | 49 | Agent designer, RAG architect, database designer, CI/CD builder, security auditor, MCP builder, AgentHub, Helm charts, Terraform, self-eval, llm-wiki (second brain for Obsidian), tc-tracker, and new C++ build/test/review/architecture skills | [engineering/](engineering/) |
...
333 CLI tools ship with the skills (all verified, stdlib-only):
...
Yes. All 333 Python CLI tools use the standard library only — zero pip installs required. Every script is verified to run with `--help`.
